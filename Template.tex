...
\section{Conclusão}
Já que o intuito desse trabalho é a analisar o modelo estocástico de Evolução Diferencial na minimização da função objetivo, logo, o trabalho apresentou bons resultados comparados a outros modelos citados na literatura, como por Oliveira\cite{claudir_tese}. Porém, a abordagem \textit{a posteriori} se mostrou desnecessária para estimativa desses parâmetros que são bem definidos segunda a análise de sensibilidade. O tempo de execução da estimativa para cada abordagem é de 45 minutos, podendo assim ser considerado o uso desse método, como eficaz e rápido, em aplicações reais pode ser uma ferramenta importante para análise e gerenciamento de sistemas LMS.
